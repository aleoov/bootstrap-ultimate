...
function eo_optionsframework_fields_custom() {

	global $allowedtags, $post;
	$options = get_option( 'eo_custom_fields_opt' );
	$option_name = 'eo_cfo';
	...
	foreach ( $options as $value ) {
		...
		//		$val = $settings[($value['id'])];
			$custom_fid = str_replace($value['id'],"_".$value['id'],$value['id']);
			//$pid = intval($_GET["post"]);
			$pid = get_the_ID();
			$val = get_post_meta( $pid, $custom_fid, true );
	...
	//		var_dump($custom_fid);
		...
		// Uploader
		case "upload":
			$output .= optionsframework_uploader_custom( $value['id'], $val, null );
...
//	echo '</div>';
// _eo-check why actually removed this ?
}