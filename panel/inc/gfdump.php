<?php 
// Google Font Dump - as of 4 / 10 / 13
$asd = '{
...
$gfarr = json_decode($asd,TRUE);
//var_dump($gfarr["items"]);

function eo_sanitize_idd($string) {
   $string = strtolower(str_replace('', '-', $string)); // Replaces all spaces with hyphens.
   return preg_replace('/[^A-Za-z0-9\-]/', '', $string); // Removes special chars.
}
		foreach ($gfarr["items"] as $gfw_family ) {
				//var_dump($gfw_family["family"]);
				$GWF_families[eo_sanitize_idd($gfw_family["family"])] = array(
		...
		$def_google_fonts = array($google_font_src_arr);	
		$chck_arr = $def_google_fonts[0];
		$serialized = serialize($chck_arr);
		$def_gf_array = unserialize($serialized);
	//	var_dump($myNewArray);
	/*	foreach ( $myNewArray as $check ) {
			//var_dump($check);
		}*/
		//print_r($def_google_fonts[0])
